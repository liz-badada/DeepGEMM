...
__global__ void mxfp4_lut_bytes_kernel(uint8_t* out) {
    ...
    const auto lut_s = deep_gemm::mxfp4::make_scaled_lut(scale);
    const uint2 lut = make_uint2(lut_s.x, lut_s.y);
    const uint2 deq = deep_gemm::mxfp4::dequant_word(q, lut_s);
...
torch::Tensor mxfp4_lut_bytes_cuda() {
    auto out = torch::empty({128, 16, 2}, torch::device(torch::kCUDA).dtype(torch::kUInt8));
    mxfp4_lut_bytes_kernel<<<8, 256>>>(out.data_ptr<uint8_t>());
    return out;
}
"""
    ...
    _CUDA_DEQUANT_EXT = load_inline(
        ...
        functions=["mxfp4_lut_bytes_cuda"],
...
def _run_cuda_dequant_lut_unit_test() -> None:
    ...
    expected = (signed * ue8m0_to_fp32(scales).view(128, 1)).to(torch.float8_e4m3fn).view(torch.uint8)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="H20 MXFP4 MegaMoE correctness gate")
